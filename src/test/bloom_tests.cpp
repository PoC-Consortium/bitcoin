...
#ifndef ENABLE_POCX
    // POCXTODO: serialized merkle block contains a CBlockHeader whose layout
    // differs under PoCX, so the upstream-derived expected byte sequence does
    // not apply. The structural checks above still exercise all merkle/filter
    // logic; only the on-wire byte comparison is skipped here.
