...
#ifdef ENABLE_POCX
                                                    /*output_amount=*/CAmount(9 * COIN), /*submit=*/false);
#else
...
#ifdef ENABLE_POCX
                                                   /*output_amount=*/CAmount(8 * COIN), /*submit=*/false);
#else
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            ...
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, CAmount(8 * COIN), false));
#else
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
...
#ifdef ENABLE_POCX
        child.vout.emplace_back(7 * COIN, spk2);
#else
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
...
#ifdef ENABLE_POCX
                                                       /*output_amount=*/CAmount(7 * COIN), /*submit=*/false);
#else
    ...
    // In-mempool parent and child with missing parent.
    {
        auto tx_parent_1 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                                            ...
                                                                            /*output_destination=*/parent_locking_script,
#ifdef ENABLE_POCX
                                                                            /*output_amount=*/CAmount(10 * COIN - low_fee_amt), /*submit=*/false));
#else
        ...
        auto tx_parent_2 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                                            ...
                                                                            /*output_destination=*/parent_locking_script,
#ifdef ENABLE_POCX
                                                                            /*output_amount=*/CAmount(10 * COIN - 800), /*submit=*/false));
#else
...
#ifdef ENABLE_POCX
                                                                                        {{9 * COIN, child_locking_script}}, /*submit=*/false));
#else
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                    ...
                                                    /*output_destination=*/parent_locking_script,
#ifdef ENABLE_POCX
                                                    /*output_amount=*/CAmount(10 * COIN) - high_fee, /*submit=*/false);
#else
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_destination=*/child_locking_script,
#ifdef ENABLE_POCX
                                                   /*output_amount=*/CAmount(10 * COIN) - 2 * high_fee, /*submit=*/false);
#else
    ...
    auto mtx_single_low_fee = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_destination=*/single_locking_script,
#ifdef ENABLE_POCX
                                                    /*output_amount=*/CAmount(9 * COIN - 1), /*submit=*/false);
#else
...
#ifdef ENABLE_POCX
    mtx_child1.vout[0].nValue = CAmount(8 * COIN);
#else
...
#ifdef ENABLE_POCX
                                                     /*output_amount=*/CAmount(9 * COIN), /*submit=*/true);
#else
...
#ifdef ENABLE_POCX
    mtx_parent2_v1.vout[0].nValue = CAmount(8 * COIN);
#else
...
#ifdef ENABLE_POCX
    mtx_mixed_child.vout.emplace_back((8 + 9 + 10 - 1) * COIN, mixed_child_spk);
#else
...
#ifdef ENABLE_POCX
    const CAmount coinbase_value{10 * COIN};
#else
