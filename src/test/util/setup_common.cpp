    ...
    : TestingSetup{ChainType::REGTEST, opts}
{
...

    {
        LOCK(::cs_main);
#ifndef ENABLE_POCX
        // PoCX: Skip hash assertion - block hashes depend on mock PoCX proof values
        // which can change during development. The chain creation itself is validated
        // by the block processing code.
        assert(
