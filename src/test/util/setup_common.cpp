...
{
    // PoCX's CBlockHeader layout differs from upstream's 80-byte header
    // (nBits/nNonce replaced by nBaseTarget/generationSignature/pocxProof),
    // so a direct `stream >> TX_WITH_WITNESS(block)` on a BTC-serialized
    // hex blob fails. Skip the 80-byte BTC header and parse only the tx
    // section into vtx, then recompute hashMerkleRoot so the block is
    // internally consistent for merkle/bloom/filter assertions.
    stream.ignore(80);
    size_t n_txs = ReadCompactSize(stream);
    block.vtx.reserve(n_txs);
