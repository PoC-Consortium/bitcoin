...
BOOST_AUTO_TEST_CASE(batch_validation_single_block)
{
    ...
    auto account_bytes = ParseHex("99BC78BA577A95A11F1A344D4D2AE55F2F857B98");
    std::copy(account_bytes.begin(), account_bytes.end(), account_id);

    uint8_t seed[32];
    auto seed_bytes = ParseHex("AFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFEAFFE");
    std::copy(seed_bytes.begin(), seed_bytes.end(), seed);
...
BOOST_AUTO_TEST_CASE(batch_validation_multiple_blocks)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        gen_sig_hex[i] = HexStr(std::span<const uint8_t>(gen_sigs[i], 32));
    }
...
BOOST_AUTO_TEST_CASE(batch_validation_mixed_compression)
{
    ...
    std::string gen_sig_hex = HexStr(std::span<const uint8_t>(gen_sig, 32));
...
BOOST_AUTO_TEST_CASE(batch_validation_8_blocks_vs_single)
{
    ...
    std::string gen_sig_hex[NUM_BLOCKS];
