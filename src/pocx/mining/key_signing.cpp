...
bool SignPoCXBlockWithKey(CBlock& block, const std::array<uint8_t, 32>& privkey_bytes)
{
    ...
    if (!key.IsValid()) {
        LogPrintf("PoCX: SignPoCXBlockWithKey: invalid private key bytes\n");
        return false;
    }

    CPubKey pubkey = key.GetPubKey();
    if (!pubkey.IsValid() || !pubkey.IsCompressed()) {
        LogPrintf("PoCX: SignPoCXBlockWithKey: derived pubkey invalid\n");
        return false;
    }
    ...
    if (!key.SignCompact(prefixed_hash, signature)) {
        LogPrintf("PoCX: SignPoCXBlockWithKey: SignCompact failed\n");
        return false;
    }
    if (signature.size() != 65) {
        LogPrintf("PoCX: SignPoCXBlockWithKey: unexpected signature size %zu\n", signature.size());
        return false;
    }
