...
PoCXScheduler::PoCXScheduler(interfaces::Mining& mining)
    ...
    // Register defensive forge callback so validation can reach us without a direct dependency
    RegisterDefensiveForgeCallback([this](const uint256& tip_hash, uint64_t incoming_quality) {
        return TryDefensiveForge(tip_hash, incoming_quality);
    });
...
PoCXScheduler::~PoCXScheduler() {
    Shutdown();
    UnregisterDefensiveForgeCallback();
...
bool PoCXScheduler::SubmitNonce(const std::string& account_id,
    ...
    // Add to queue with DoS protection
    {
        ...
        if (m_submission_queue.size() >= MAX_QUEUE_SIZE) {
            LogInfo("PoCX: [Scheduler] Submission queue full (%zu entries), rejecting submission\n",
                     MAX_QUEUE_SIZE);
            return false;
        }
...
void PoCXScheduler::Shutdown() {
    ...
    LogInfo("PoCX: [Scheduler] Shutting down worker thread\n");
    ...
    LogInfo("PoCX: [Scheduler] Shutdown complete\n");
}
...
bool PoCXScheduler::TryDefensiveForge(const uint256& tip_hash, uint64_t incoming_quality) {
    ...
    if (!m_current_forging) {
        LogInfo("PoCX: [DefensiveCheck] Incoming quality: %llu, Our best: (none) -> ACCEPTED\n",
                 incoming_quality);
        return false;
    }

    if (m_current_forging->tip_block_hash != tip_hash) {
        LogInfo("PoCX: [DefensiveCheck] Incoming quality: %llu, Tip mismatch (ours: %s, theirs: %s) -> ACCEPTED\n",
                 incoming_quality,
    ...
    if (our_quality >= incoming_quality) {
        ...
        LogInfo("PoCX: [DefensiveCheck] Incoming quality: %llu, Our best: %llu -> ACCEPTED\n",
    ...
    LogInfo("PoCX: [DefensiveCheck] Incoming quality: %llu, Our best: %llu -> DEFENDING (%ld sec early)\n",
    ...
    while (!m_shutdown.load()) {
        try {
            ...
            if (m_defensive_forge_requested.exchange(false)) {
                LogInfo("PoCX: [Scheduler] RUSHING BLOCK for defensive forging!\n");
                ...
                if (should_forge) {
                    ...
                    if (success) {
                        LogInfo("PoCX: [Scheduler] Defensive forge SUCCESS\n");
                    } else {
                        LogInfo("PoCX: [Scheduler] Defensive forge FAILED\n");
                    }

                    std::lock_guard<std::mutex> lock(m_forging_mutex);
                    m_current_forging.reset();
                } else {
                    LogInfo("PoCX: [Scheduler] Defensive forge skipped - no valid forging state\n");
                }
                continue;
            }
            ...
            LogInfo("PoCX: [Scheduler] Exception in worker thread: %s\n", e.what());
            std::lock_guard<std::mutex> lock(m_forging_mutex);
            m_current_forging.reset();
        }
    }

    LogInfo("PoCX: [Scheduler] Worker thread stopped\n");
}
...
void PoCXScheduler::ProcessSubmission(const NonceSubmission& submission) {
    ...
    auto current_context = pocx::mining::GetNewBlockContext(*node_context->chainman);
    ...
    // Check if better and update state (under lock)
    {
        ...
        LogInfo("PoCX: [Scheduler] New best solution - tip: %s, quality: %llu, deadline: %llu sec\n",
...
void PoCXScheduler::WaitForDeadlineOrNewSubmission() {
    ...
    if (deadline_reached) {
        ...
        std::lock_guard<std::mutex> lock(m_forging_mutex);
        LogInfo("PoCX: [Scheduler] Deadline %lus -> %s\n", deadline, success ? "completed" : "failed");
        m_current_forging.reset();
        if (success) {
            LogInfo("PoCX: [Scheduler] State reset for new block competition\n");
        }
    }
}

bool PoCXScheduler::ForgeBlock(bool defensive) {
    LogInfo("PoCX: [Scheduler] ForgeBlock started%s\n", defensive ? " (defensive)" : "");
    ...
    if (!m_current_forging) {
        LogInfo("PoCX: [Scheduler] No current forging state, returning false\n");
        return false;
    }
    ...
    if (!context) {
        LogInfo("PoCX: [Scheduler] No node context available\n");
        return false;
    }
    ...
    if (!block) {
        LogInfo("PoCX: [Scheduler] Block building failed\n");
        return false;
    }

    LogInfo("PoCX: [Scheduler] Block built, starting signing process\n");
    ...
    if (defensive) {
        ...
        if (drift > 0) {
            block->nTime += drift;
            LogInfo("PoCX: [Scheduler] Adjusted block timestamp by +%ld seconds for defensive forging\n", drift);
        }
    }
    ...
    if (context->wallet_loader) {
        ...
        LogInfo("PoCX: [Scheduler] Trying %zu wallet(s) for effective signer %s\n",
        ...
        LogInfo("PoCX: [Scheduler] No wallet loader available for signing\n");
    }

    if (!signed_successfully) {
        LogInfo("PoCX: [Scheduler] Block signing failed\n");
        return false;
    }

    LogInfo("PoCX: [Scheduler] Block forged with nonce: %llu, quality: %llu, compression: %u\n",
...
bool PoCXScheduler::SubmitForgedBlock(const CBlock& block)
{
    ::node::NodeContext* context = m_mining->context();
    if (!context || !context->chainman) {
        LogInfo("PoCX: [Scheduler] Failed to get chainstate manager for block submission\n");
        return false;
    }
    ...
    LogInfo("PoCX: [Scheduler] Submitting forged block (hash: %s) to Bitcoin Core\n", block.GetHash().ToString());
    ...
    if (accepted) {
        LogInfo("PoCX: [Scheduler] Block forged and accepted! Hash: %s, New: %s\n",
                 block.GetHash().ToString(),
                 new_block ? "true" : "false");

        // Log the new chain tip to confirm block was added
        auto new_tip = CHECK_NONFATAL(m_mining->getTip()).value();
        LogInfo("PoCX: [Scheduler] New chain tip - Hash: %s, Height: %d\n",
                 new_tip.hash.ToString(), new_tip.height);
    } else {
        LogInfo("PoCX: [Scheduler] Block forged but rejected\n");
    }
