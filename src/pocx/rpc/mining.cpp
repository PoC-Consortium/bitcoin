...
static void EnsurePoCXScheduler(interfaces::Mining& mining) {
    ...
    if (!g_pocx_scheduler) {
        ...
        LogInfo("PoCX: Scheduler initialized\n");
    }
}
#endif
    ...
    return RPCHelpMan{"submit_nonce",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            try {
                ...
                if (node.wallet_loader) {
                    ...
                    // Check for assignments to get the effective signer
                    {
                        ...
                        if (effective_signer_account != account_id) {
                            LogInfo("PoCX: Plot %s has assignment, checking key for effective signer: %s\n",
                ...
                LogInfo("PoCX: nonce=%llu height=%d gensig=...%s account=...%s seed=...%s raw_quality=%llu forge_time=%lus -> ACK\n",
                             nonce, height,
