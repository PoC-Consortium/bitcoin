...
//
// SSE2 implementation of Shabal256 - processes 4 independent streams in parallel.
...
// Shabal256 initial state constants (same as scalar/AVX2 version)
alignas(16) static const uint32_t A_INIT_SSE2[12] = {
...
alignas(16) static const uint32_t B_INIT_SSE2[16] = {
...
alignas(16) static const uint32_t C_INIT_SSE2[16] = {
...
static inline __m128i add_epi32(__m128i a, __m128i b) {
    return _mm_add_epi32(a, b);
}

static inline __m128i sub_epi32(__m128i a, __m128i b) {
    return _mm_sub_epi32(a, b);
}

static inline __m128i xor_si128(__m128i a, __m128i b) {
    return _mm_xor_si128(a, b);
}

static inline __m128i andnot_si128(__m128i a, __m128i b) {
    return _mm_andnot_si128(a, b);
}

static inline __m128i or_si128(__m128i a, __m128i b) {
    return _mm_or_si128(a, b);
}

// Rotate left by n bits
static inline __m128i rotl(__m128i x, int n) {
    return or_si128(_mm_slli_epi32(x, n), _mm_srli_epi32(x, 32 - n));
}

// Load 4 uint32_t values from 4 different sources at the same offset
static inline __m128i load4(const uint32_t* src[4], size_t offset) {
...
// Load 4 bytes from 4 different byte arrays and convert to __m128i of uint32_t
static inline __m128i load4_bytes(const uint8_t* src[4], size_t byte_offset) {
    uint32_t vals[4];
    for (int i = 0; i < 4; i++) {
        if (src[i]) {
    ...
    return _mm_set_epi32(vals[3], vals[2], vals[1], vals[0]);
}

// Store 4 uint32_t values to 4 different destinations
static inline void store4(uint8_t* dst[4], size_t byte_offset, __m128i val) {
...
// Broadcast single uint32_t to all 4 lanes
static inline __m128i broadcast(uint32_t val) {
    return _mm_set1_epi32(static_cast<int>(val));
}

// SSE2 doesn't have _mm_mullo_epi32, need to emulate it
static inline __m128i mullo_epi32(__m128i a, __m128i b) {
    // Multiply even and odd 32-bit integers separately
    __m128i tmp1 = _mm_mul_epu32(a, b);                    // multiply elements 0, 2
    __m128i tmp2 = _mm_mul_epu32(_mm_srli_si128(a, 4), _mm_srli_si128(b, 4)); // multiply elements 1, 3
    // Combine results: take low 32 bits from each 64-bit result
...
// Shabal permutation element macro (PP128 equivalent)
#define PERM_ELT(a, b, xa0, xa1, xb0, xb1, xb2, xb3, xc, xm) do { \
    __m128i tmp = rotl(a[xa1], 15); \
    tmp = mullo_epi32(tmp, broadcast(5)); \
    a[xa0] = xor_si128(a[xa0], tmp); \
    a[xa0] = xor_si128(a[xa0], xc); \
    a[xa0] = mullo_epi32(a[xa0], broadcast(3)); \
    a[xa0] = xor_si128(a[xa0], b[xb1]); \
    a[xa0] = xor_si128(a[xa0], andnot_si128(b[xb3], b[xb2])); \
    a[xa0] = xor_si128(a[xa0], xm); \
    __m128i tmp2 = rotl(b[xb0], 1); \
    b[xb0] = _mm_xor_si128(tmp2, a[xa0]); \
    b[xb0] = _mm_xor_si128(b[xb0], _mm_set1_epi32(-1)); \
} while(0)

// Full permutation (48 elements, 3 rounds of 16)
static inline void perm(__m128i a[12], __m128i b[16], const __m128i c[16], const __m128i m[16]) {
...
static inline void apply_round(__m128i a[12], __m128i b[16], __m128i c[16],
                        const __m128i m[16], __m128i w_low, __m128i w_high) {
    // B = B + M
    for (int i = 0; i < 16; i++) {
    ...
    a[0] = xor_si128(a[0], w_low);
    a[1] = xor_si128(a[1], w_high);
...
static inline void apply_p(__m128i a[12], __m128i b[16], const __m128i c[16], const __m128i m[16]) {
    // Rotate B left by 17
    for (int i = 0; i < 16; i++) {
        b[i] = rotl(b[i], 17);
    }
...
void Shabal256_sse2(
    ...
    // Initialize state - broadcast initial values to all 4 lanes
    __m128i a[12], b[16], c[16];
    for (int i = 0; i < 12; i++) {
        a[i] = broadcast(A_INIT_SSE2[i]);
    }
    for (int i = 0; i < 16; i++) {
        b[i] = broadcast(B_INIT_SSE2[i]);
        c[i] = broadcast(C_INIT_SSE2[i]);
    }

    // Message counters (same for all lanes)
    __m128i w_low = broadcast(1);
    __m128i w_high = broadcast(0);

    // Process data blocks (64 bytes each)
    size_t num_blocks = len >> 6;
    for (size_t block = 0; block < num_blocks; block++) {
        // Load message block from all 4 sources
        __m128i m[16];
        for (int i = 0; i < 16; i++) {
            m[i] = load4_bytes(data, block * 64 + i * 4);
        }
