    ...
    for (auto it = blockUndo.vforgingundo.rbegin(); it != blockUndo.vforgingundo.rend(); ++it) {
        ...
        switch (undo.type) {
            case ForgingUndo::UndoType::ADDED:
                ...
                LogInfo("PoCX: Reorg - Removed assignment plot=%s txid=%s\n",
            ...
            case ForgingUndo::UndoType::REVOKED:
                ...
                LogInfo("PoCX: Reorg - Restored assignment plot=%s txid=%s (un-revoked)\n",
                         HexStr(undo.assignment.plotAddress),
                         undo.assignment.assignment_txid.ToString());
                break;
            ...
            case ForgingUndo::UndoType::MODIFIED:
                ...
                LogInfo("PoCX: Reorg - Restored modified assignment plot=%s txid=%s\n",
                         HexStr(undo.assignment.plotAddress),
                         undo.assignment.assignment_txid.ToString());
                break;
        }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        for (size_t output_idx = 0; output_idx < tx.vout.size(); output_idx++) {
            ...
            if (pocx::assignments::IsAssignmentOpReturn(output)) {
                ...
                LogInfo("PoCX: Assignment OP_RETURN found - tx=%s plot=%s forge=%s\n",
                ...
                LogInfo("PoCX: Assignment created - plot=%s forge=%s txid=%s activates=%d\n",
            ...
            // Check for revocation OP_RETURN (XCOP marker)
                ...
                LogInfo("PoCX: Revocation OP_RETURN found - tx=%s plot=%s\n",
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (block.nBaseTarget != expected_base_target) {
        LogInfo("PoCX: Base target mismatch - block has %llu, expected %llu\n",
        ...
        std::vector<const CBlockHeader*> headers_to_validate;
        {
            ...
            for (const auto& header : headers) {
                ...
                if (const CBlockIndex* known = m_blockman.LookupBlockIndex(header.GetHash())) {
                    if (known->nStatus & BLOCK_FAILED_VALID) {
                        return state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID, "duplicate-invalid",
